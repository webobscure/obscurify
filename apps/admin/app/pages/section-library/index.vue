<template>
  <div>
    <PageHeader title="Section Library" />
    ...
    <template v-else>
      <p class="hint">
        Every built-in section type, seeded onto the active theme the first
        time it's needed. Insert one onto a page from the
        <NuxtLink to="/pages">Pages</NuxtLink> list's own Builder.
      </p>
...
async function load() {
  ...
  error.value = null
  try {
    const response = await useApi().builder.presets.list('section')
